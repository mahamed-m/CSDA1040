...
*****************Explain what info the above code outputs, and how it is used to determine the sample selection below.
...
**********************Describe what the plot(s) show.



# Limitations

During the planning and implementation of the project we encountered limitations due to the size of the data and time needed to train our model. It was our initial intention to apply Transfer Learning to our data set, using model 1 model 2 and model 3 and compare the accuracy. The model with the greatest accuracy was to be used in the implementation of our Shiny app. The data was successfully trained using the VGG16 pre-trained model, obtaining an accuracy of......

Since most image recognition examples were built in Python, we considered trying using the R equivalent to show how it can also be done in R. This did, however, present us with a number of difficulties in running it, as calls to Python needed to be  made.... 

Unltimately we opted to build the model from scratch using a subset of the original image classes. This allowed use to .....*****What were the benefits of this. Using this model we were able to improve the prediction accuracy by......over that of transfer learning.



# Shiny App

We created an app that would allow users to gauge whether their photos show expressions which tend to be "Happy" or "Sad". The app, called "Facial Expression Recognition Test" can be found at ...(add url) and its functionality is described below.

With the app, users can upload a greyscale image file. Using the browse button, users can search for images from their desktop libraries. Once the appropriate image, in the jpeg or png format, is selected, the app automatically uploads the image. Upon image upload, the model is activated and processes the image to determine facial expression.

There are three aspects to this part. First, the app will display the image that was uploaded into Shiny. This allows users to view their images in consideration with what the model reveals. Secondly, the app will display a percentage. This percentage represents the model's predicted expression. Lastly, in conjunction with the displayed percentage, the Shiny will output a statement telling the user whether the image they uploaded tends to a "Happy" or "Sad" expression. The statement choice depends on the model's predictions. The model provides two prediction percentages for "Happy" or "Sad". With a benchmark of 50%, the provided output statement determines the more noticeable expression.

**** Not sure if this part below belongs here or in the limitations section ****

An interesting thing to note regarding the model predictions and image processing is exactly how the model views images. A face contains a multitude of expressions, sometimes ranging between emotions, and the model seems to pick up on that aspect of facial recognition. For example, the model seems to analyze different points such as the eyes and the mouth, and take those aspects into consideration when predicting expression. An image can then be assigned a percentage under another category, yet present mainly as belonging to a specific category because of the 50% benchmark assigned.
